...
extern "C" __global__ void __closesthit__RayRadianceLeft()
{
...
#ifdef USE_Correlation
	float3 rayDirRight = paras.trans->r0Right - hitPointPosition;
	...
	float2 dirRightScreenSpace
	{
		dot(paras.trans->proj0Right,dirRightEyeSpace),
		dot(paras.trans->proj1Right,dirRightEyeSpace)
	};
	dirRightScreenSpace = (1 - dirRightScreenSpace / dirRightEyeSpace.z) / 2;
	int2 idxRight2{ roundf(dirRightScreenSpace.x * paras.size.x), roundf(dirRightScreenSpace.y * paras.size.y) };
	unsigned int idxRight(idxRight2.y * paras.size.x + idxRight2.x);
	bool accessibleToRightEye(false);
	if (idxRight2.x < paras.size.x && idxRight2.x >= 0 && idxRight2.y < paras.size.y && idxRight2.y >= 0)
	{
		rayDirRight /= rMax;
		if (*(int*)&paras.imageRight[idxRight].w == -1)
		{
			unsigned int pd0(1);
			optixTrace(paras.handle, hitPointPosition, rayDirRight,
			...
			if (pd0)
			{
				...
				{
					//*(int*)&paras.imageRight[idxRight].w = idxLeft;
	...
	//used for same point different face
	...
	//else
	{
		...
		for (unsigned int c0(0); c0 < paras.lightSourceNum; ++c0)
		{
			...
			if (cosDN * rayDotNormalLeft < 0 &&
				-dot(lightSource.direction, shadowRayDir) > lightSource.visiableAngle)
			{
				unsigned int pd0(1);
				optixTrace(paras.handle, hitPointPosition, shadowRayDir,
					0.0001f, Tmax,
	...
	*(float3*)&paras.imageLeft[idxLeft] = directFluxLeft;
	...
	{
		*(float3*)&paras.imageRight[idxRight] = { 0,0.7,0 };//directFluxRight;
